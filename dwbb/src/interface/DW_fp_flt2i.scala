...
package org.chipsalliance.dwbb.interface.DW_fp_flt2i
...
case class Parameter(
  ...
  require(
    Range.inclusive(3, 512).contains(iSize),
    "Integer size must between 3 and 512"
  )
}
...
class Interface(parameter: Parameter) extends Bundle {
  ...
  val z: UInt = Output(UInt(parameter.iSize.W))
