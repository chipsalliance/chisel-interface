...
package org.chipsalliance.dwbb.interface.DW_fp_i2flt
...
case class Parameter(
  ...
  require(
    Range.inclusive(3 + (if (iSign) 1 else 0), 512).contains(iSize),
    s"Integer size must between ${3 + (if (iSign) 1 else 0)} and 512"
  )
}

class Interface(parameter: Parameter) extends Bundle {
  val a: UInt = Input(UInt(parameter.iSize.W))
