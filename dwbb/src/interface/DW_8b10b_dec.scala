...
package org.chipsalliance.dwbb.interface.DW_8b10b_dec
...
class Interface(parameter: Parameter) extends Bundle {
  ...
  val rd_err_bus: UInt = Output(UInt(parameter.bytes.W))
  val code_err_bus: UInt = Output(UInt(parameter.bytes.W))
}
